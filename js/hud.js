...
// Vinger-tubes: cosinusprofiel → cilindrisch licht-effect
// bi < 18 = vingerbotten, bi >= 18 = palmverbindingen
const _BONE_PARTS = HAND_CONNECTIONS.map(([a, b], bi) => {
  const isFinger = bi < 18;
  const N      = isFinger ? 130 : 40;
  const SPREAD = isFinger ? 0.38 : 0.10;
  return Array.from({ length: N }, (_, p) => {
    ...
    const cosF    = Math.cos(absFrac * Math.PI * 0.5); // 1.0→0.0, cilindrisch profiel
    const isCore  = absFrac < 0.22;                  // centrale highlight
    const tier = (bi * 7 + p * 17) % 10;
    const baseSz = tier >= 9 ? 1.6 + ((bi * 3 + p) % 6) / 3
                 : tier >= 7 ? 0.8 + ((bi * 5 + p) % 7) / 8
                 :             0.35 + ((bi * 7 + p * 11) % 7) / 14;
    const baseAl = tier >= 9 ? 0.75 + ((bi + p * 3) % 20) / 100
                 : tier >= 7 ? 0.42 + ((bi * 3 + p) % 28) / 100
                 :             0.14 + ((bi * 3 + p * 7) % 26) / 100;
    return {
      t, perp,
      sz: baseSz * (0.55 + cosF * 0.65) * (isCore ? 1.25 : 1.0),
      al: baseAl * cosF * cosF * (isCore ? 1.35 : 1.0),
      gold: (bi * 13 + p * 23) % 8 === 0,
    };
  });
});
